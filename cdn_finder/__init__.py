...
class RemoteFinder(BaseFinder):
    def __init__(self):
        ...
        self.cdn_prefix= getattr(settings, "REMOTE_FINDER_RESOURCES", None)
    ...
    def find(self, path, all=False):
        ...
        # except KeyError:
        #     return []
        resolve_path=self.resolve(path)
        self.fetch(path,resolve_path)
        match = self.storage.path(resolve_path)
        if all:
            return [match]
        else:
            return match
    ...
    def fetch(self, path,save_path):
        ...
        response = urlopen(self.cdn_prefix+path)
        content = None
